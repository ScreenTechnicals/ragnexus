...
const App = () => {
    ...
    const handleUrlSubmit = useCallback(async (raw: string) => {
        if (!ai) { setError('❌ Missing GEMINI_API_KEY.'); return; }
    ...
    const handleChat = useCallback(async (query: string) => {
        ...
        try {
            const genkitConfig = await genkitAdapter.getGenerateOptions(
                { model: gemini('gemini-2.0-flash'), messages: newMessages as any },
                { memory: false }
            );
    ...
    if (step === 'url-input') return (
        <Box flexDirection="column" gap={1} paddingY={1}>
            <Box flexDirection="column" borderStyle="round" borderColor="blueBright" paddingX={2} paddingY={1}>
                <Text color="blueBright" bold>🔷 RagNexus — Genkit + Gemini</Text>
    ...
    if (step === 'scraping' || step === 'embedding') return (
        <Box flexDirection="column" gap={1} paddingY={1}>
            <Box gap={1}>
                ...
                <Text color="blueBright">{statusMsg}</Text>
            </Box>
    ...
    return (
        <Box flexDirection="column" gap={1} paddingY={1}>
            <Box flexDirection="column" borderStyle="round" borderColor="blueBright" paddingX={2} paddingY={0}>
                <Box gap={2}>
                    <Text color="blueBright" bold>🔷 RagNexus — Genkit</Text>
                    <Text color="gray">{docCount} chunks · {loadedUrls.length} source(s)</Text>
                </Box>
                {loadedUrls.map(u => <Text key={u} color="gray" dimColor>  {shortUrl(u)}</Text>)}
            </Box>
            <Text color="gray" dimColor>  Tip: <Text color="blueBright">/add &lt;url&gt;</Text> to load more · <Text color="blueBright">exit</Text> to quit</Text>

            <Box flexDirection="column" marginTop={1}>
                {messages.filter(m => m.role !== 'system').map((m, i) => (
                    <Box key={i} flexDirection="column" marginBottom={1}>
                        <Text color={m.role === 'user' ? 'blueBright' : 'cyanBright'} bold>
            ...
            {isGenerating && (
                <Box flexDirection="column" marginBottom={1}>
                    <Text color="cyanBright" bold>Genkit</Text>
